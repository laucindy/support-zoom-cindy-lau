...
class AsciiArtTest < Minitest::Test
  ...
  def test_outputs_triangle_of_asterisks_center_aligned
  ...
  def test_empty_output_triangle_of_asterisks_center_aligned
    ascii_art = AsciiArt.new.extend CenterTriangle
    expected = "\n"
    assert_output(expected) { puts ascii_art.draw(0) }
  end

  def test_outputs_triangle_of_asterisks_diamond_shape
  ...
  def test_empty_output_triangle_of_asterisks_diamond_shape
    ascii_art = AsciiArt.new.extend DiamondTriangle
    expected = "\n"
    assert_output(expected) { puts ascii_art.draw(0) }
  end
